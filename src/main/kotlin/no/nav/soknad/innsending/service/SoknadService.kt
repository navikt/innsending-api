...
@Service
open class SoknadService(private val skjemaService: HentSkjemaDataConsumer, private val soknadRepository: SoknadRepository, private val vedleggRepository: VedleggRepository) {
    ...
    open fun opprettSoknad(brukerId: String, skjemanr: String, spraak: String = "no"): DokumentSoknadDto {
        ...
        val soknadDbData = soknadRepository.save(SoknadDbData(null, UUID.randomUUID().toString(), kodeverkSkjema.tittel ?: "", kodeverkSkjema.skjemanummer ?: "", kodeverkSkjema.tema ?: "", spraak, SoknadsStatus.Opprettet, brukerId, null
            , LocalDateTime.now(), LocalDateTime.now(),  null))

        val vedleggDbData = vedleggRepository.save(VedleggDbData(null,kodeverkSkjema.tittel ?: "", kodeverkSkjema.skjemanummer ?: kodeverkSkjema.vedleggsid, kodeverkSkjema.url, null, OpplastingsStatus.IkkeLastetOpp
            , true, false, true,UUID.randomUUID().toString(), null, soknadDbData.id ?: 1L
    ...
    fun hentSoknad(id: Long): DokumentSoknadDto {
        ...
        if (soknadDbDataOpt.isPresent) {
            ...
            val vedleggDtoListe = vedleggDbDataListe.stream().map { v -> lagVedleggDto(v) }.toList()
    ...
    open fun opprettEllerOppdaterSoknad(dokumentSoknadDto: DokumentSoknadDto): DokumentSoknadDto {
        ...
        dokumentSoknadDto.vedleggsListe.stream().map { v -> mapTilVedleggDb(v, soknadsid!!) }.forEach { m -> vedleggRepository.save(m) }
    ...
    fun slettVedlegg(vedleggDto: VedleggDto, soknadsId: Long) {
        ...
        if (!vedleggDto.erHoveddokument && !"pdf".equals(vedleggDto.mimetype)) {
    ...
    private fun lagVedleggDto(vedleggDbData: VedleggDbData): VedleggDto {
        return VedleggDto(vedleggDbData.id!!, vedleggDbData.vedleggsnr, vedleggDbData.tittel, vedleggDbData.skjemaurl
            , vedleggDbData.uuid, vedleggDbData.mimetype, vedleggDbData.dokument, vedleggDbData.erhoveddokument, vedleggDbData.ervariant, vedleggDbData.erpdfa, vedleggDbData.status
            , vedleggDbData.opprettetdato)
    }

    private fun lagDokumentSoknadDto(soknadDbData: SoknadDbData, vedleggDtoListe: List<VedleggDto>): DokumentSoknadDto {
        return DokumentSoknadDto(soknadDbData.id ?: 1L, soknadDbData.behandlingsid, soknadDbData.ettersendingsid, soknadDbData.brukerid, soknadDbData.skjemanr, soknadDbData.tittel, soknadDbData.tema, soknadDbData.spraak, soknadDbData.status
            , soknadDbData.opprettetdato, soknadDbData.endretdato, soknadDbData.innsendtdato
            , vedleggDtoListe)
    }

    private fun mapTilVedleggDb(vedleggDto: VedleggDto, soknadsId: Long): VedleggDbData {
        return VedleggDbData(vedleggDto.id, vedleggDto.tittel, vedleggDto.vedleggsnr, vedleggDto.skjemaurl, vedleggDto.mimetype, vedleggDto.opplastingsStatus
            , vedleggDto.erHoveddokument, vedleggDto.erVariant, vedleggDto.erPdfa, vedleggDto.uuid, vedleggDto.document, soknadsId, vedleggDto.opprettetdato, LocalDateTime.now())
    }

    private fun mapTilSoknadDb(dokumentSoknadDto: DokumentSoknadDto): SoknadDbData {
        return SoknadDbData(dokumentSoknadDto.id, dokumentSoknadDto.behandlingsId ?: UUID.randomUUID().toString(), dokumentSoknadDto.tittel, dokumentSoknadDto.skjemanr, dokumentSoknadDto.tema, dokumentSoknadDto.spraak, dokumentSoknadDto.status, dokumentSoknadDto.brukerId, dokumentSoknadDto.ettersendingsId
