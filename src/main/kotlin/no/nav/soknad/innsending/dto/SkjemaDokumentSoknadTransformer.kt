...
class SkjemaDokumentSoknadTransformer(private val input: SkjemaDto) {
	...
	private fun SkjemaDto.toDokumentSoknadDto() = DokumentSoknadDto(null, null, null, brukerId,
		skjemanr, tittel, tema, spraak, null, SoknadsStatus.Opprettet, LocalDateTime.now(), LocalDateTime.now(), null, konverterTilvedleggsListe(vedleggsListe) )
	...
	private fun SkjemaDokumentDto.toVedleggDto() = VedleggDto(null, vedleggsnr, tittel, null, mimetype, document,
		erHoveddokument, erVariant, erPdfa ?: false, toOpplastingsstatus(opplastingsStatus) ?: OpplastingsStatus.IKKE_VALGT, LocalDateTime.now() )
