...
private val defaultProperties = ConfigurationMap(
	mapOf(
		...
		"SANITY_HOST" to "http://localhost:8080",
		"SANITY_ENDPOINT" to "/soknadsveiviserproxy/skjemautlisting",
...
data class AppConfiguration(val restConfig: RestConfig = RestConfig(), val dbConfig: DBConfig = DBConfig(), val kafkaConfig: KafkaConfig = KafkaConfig()) {
	...
	data class RestConfig(
		...
		val sharedUsername: String = readFileAsText("secret/team-soknad/innsending/basicauth/username", "SHARED_USERNAME".configProperty()),
		val sharedPassword: String = readFileAsText("secret/team-soknad/innsending/basicauth/password", "SHARED_PASSWORD".configProperty()),
		val maxFileSize: Int = "MAX_FILE_SIZE".configProperty().toInt(),
		val sanityHost: String = "SANITY_HOST".configProperty(),
		val sanityEndpoint: String = "SANITY_ENDPOINT".configProperty(),
