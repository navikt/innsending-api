...
interface VedleggRepository : JpaRepository<VedleggDbData, Long> {
	...
	@Query(value = "FROM VedleggDbData WHERE soknadsid = :soknadsid and status = :status and endretdato >= :fromDate and not (vedleggsnr='L7' or erhoveddokument=true) order by id")
	@Modifying(clearAutomatically = true)
	fun findAllBySoknadsidAndStatusAndAfterInnsendtdato(@Param("soknadsid") soknadsid: Long, @Param("status") status: OpplastingsStatus, @Param ("fromDate") fromDate: LocalDateTime): List<VedleggDbData>
