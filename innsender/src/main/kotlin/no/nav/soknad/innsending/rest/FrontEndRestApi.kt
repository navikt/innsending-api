...
class FrontEndRestApi(
	...
	override fun opprettSoknad(opprettSoknadBody: OpprettSoknadBody): ResponseEntity<DokumentSoknadDto> {
		...
		loggBegge("Skal opprette søknad for ${opprettSoknadBody.skjemanr}", brukerId)
		...
		loggBegge("${dokumentSoknadDto.innsendingsId}: Opprettet søknad på skjema ${opprettSoknadBody.skjemanr}", brukerId)
	...
	override fun ettersendingPaInnsendingsId(opprettEttersendingGittInnsendingsId: OpprettEttersendingGittInnsendingsId): ResponseEntity<DokumentSoknadDto> {
		...
		loggBegge(
			"Kall for å opprette ettersending på søknad ${opprettEttersendingGittInnsendingsId.ettersendingTilinnsendingsId}",
			brukerId
		)
		...
		loggBegge(
			"${dokumentSoknadDto.innsendingsId}: Opprettet ettersending for innsendingsid ${opprettEttersendingGittInnsendingsId.ettersendingTilinnsendingsId}",
			brukerId
		)
	...
	override fun opprettEttersendingGittSkjemanr(opprettEttersendingGittSkjemaNr: OpprettEttersendingGittSkjemaNr): ResponseEntity<DokumentSoknadDto> {
		val brukerId = tilgangskontroll.hentBrukerFraToken()
		loggBegge("Kall for å opprette ettersending på skjema ${opprettEttersendingGittSkjemaNr.skjemanr}", brukerId)
	...
	override fun hentAktiveOpprettedeSoknader(): ResponseEntity<List<DokumentSoknadDto>> {
		...
		brukerIds.forEach { loggBegge("Kall for å hente alle opprette ikke innsendte søknader", it) }
		val dokumentSoknadDtos = soknadService.hentAktiveSoknader(brukerIds)
		brukerIds.forEach { loggBegge("Hentet ${dokumentSoknadDtos.size} søknader opprettet av bruker", it) }
	...
	override fun hentSoknad(innsendingsId: String): ResponseEntity<DokumentSoknadDto> {
		val brukerId = tilgangskontroll.hentBrukerFraToken()
		loggBegge("$innsendingsId: Kall for å hente søknad", brukerId)
		val soknadDto = hentOgValiderSoknad(innsendingsId)
		loggBegge("$innsendingsId: Hentet søknad", brukerId)
	...
	override fun hentSoknaderForSkjemanr(
		...
		if (soknadstyper.isNullOrEmpty()) {
			brukerIds.forEach { loggBegge("Henter søknader med alle søknadstyper for $skjemanr", it) }
		...
		if (soknadstyper?.contains(SoknadType.soknad) == true) {
			brukerIds.forEach { loggBegge("Henter søknader med søknadstype 'soknad' for $skjemanr", it) }
		...
		if (soknadstyper?.contains(SoknadType.ettersendelse) == true) {
			brukerIds.forEach { loggBegge("Henter søknader med søknadstype 'ettersendelse' for $skjemanr", it) }
	...
	override fun endreSoknad(innsendingsId: String, patchSoknadDto: PatchSoknadDto): ResponseEntity<Unit> {
		...
		loggBegge("$innsendingsId: Kall for å endre søknad", brukerId)

		val soknadDto = hentOgValiderSoknad(innsendingsId)
		soknadService.endreSoknad(soknadDto.id!!, patchSoknadDto.visningsSteg)

		loggBegge("$innsendingsId: Oppdatert søknad", brukerId)
	...
	override fun hentVedleggsListe(innsendingsId: String): ResponseEntity<List<VedleggDto>> {
		...
		loggBegge("$innsendingsId: Kall for å vedleggene til søknad", brukerId)

		val soknadDto = hentOgValiderSoknad(innsendingsId)
		val vedleggsListeDto = soknadDto.vedleggsListe

		loggBegge("$innsendingsId: Hentet vedleggene til søknad", brukerId)
	...
	override fun hentVedlegg(innsendingsId: String, vedleggsId: Long): ResponseEntity<VedleggDto> {
		...
		loggBegge("$innsendingsId: Kall for å hente vedlegg $vedleggsId til søknad", brukerId)
		...
		loggBegge("$innsendingsId: Hentet vedlegg $vedleggsId til søknad", brukerId)
	...
	override fun endreVedlegg(
		...
		loggBegge("$innsendingsId: Lagret vedlegg ${vedleggDto.id} til søknad", brukerId)
	...
	override fun lagreVedlegg(innsendingsId: String, postVedleggDto: PostVedleggDto?): ResponseEntity<VedleggDto> {
		...
		loggBegge("$innsendingsId: Kall for å lagre vedlegg til søknad", brukerId)
	...
	override fun lagreFil(innsendingsId: String, vedleggsId: Long, file: Resource): ResponseEntity<FilDto> {
		...
		loggBegge("$innsendingsId: Kall for å lagre fil på vedlegg $vedleggsId til søknad", brukerId)
		...
		loggBegge("$innsendingsId: Lagret fil ${lagretFilDto.id} på vedlegg $vedleggsId til søknad", brukerId)
	...
	override fun hentFil(innsendingsId: String, vedleggsId: Long, filId: Long): ResponseEntity<Resource> {
		...
		loggBegge("$innsendingsId: Kall for å hente fil $filId på vedlegg $vedleggsId til søknad", brukerId)
		...
		loggBegge("$innsendingsId: Hentet fil ${filDto.id} på vedlegg $vedleggsId til søknad", brukerId)
	...
	override fun hentFilInfoForVedlegg(innsendingsId: String, vedleggsId: Long): ResponseEntity<List<FilDto>> {
		...
		loggBegge("$innsendingsId: Kall for å hente filinfo til vedlegg $vedleggsId til søknad", brukerId)

		val soknadDto = hentOgValiderSoknad(innsendingsId)
		val filDtoListe = filService.hentFiler(soknadDto, innsendingsId, vedleggsId)

		loggBegge("$innsendingsId: Hentet informasjon om opplastede filer på vedlegg $vedleggsId til søknad", brukerId)
	...
	override fun slettFil(innsendingsId: String, vedleggsId: Long, filId: Long): ResponseEntity<VedleggDto> {
		...
		loggBegge("Kall for å slette fil $filId på vedlegg $vedleggsId til søknad $innsendingsId", brukerId)

		val soknadDto = hentOgValiderSoknad(innsendingsId)
		val vedleggDto = filService.slettFil(soknadDto, vedleggsId, filId)

		loggBegge("$innsendingsId: Slettet fil $filId på vedlegg $vedleggsId til søknad", brukerId)
	...
	override fun slettVedlegg(innsendingsId: String, vedleggsId: Long): ResponseEntity<BodyStatusResponseDto> {
		...
		loggBegge("$innsendingsId: Kall for å slette vedlegg $vedleggsId for søknad", brukerId)

		val soknadDto = hentOgValiderSoknad(innsendingsId)
		vedleggService.slettVedlegg(soknadDto, vedleggsId)

		loggBegge("$innsendingsId: Slettet vedlegg $vedleggsId for søknad", brukerId)
	...
	override fun slettSoknad(innsendingsId: String): ResponseEntity<BodyStatusResponseDto> {
		...
		loggBegge("$innsendingsId: Kall for å slette søknad", brukerId)
	...
	override fun sendInnSoknad(innsendingsId: String): ResponseEntity<KvitteringsDto> {
		...
		loggBegge("$innsendingsId: Kall for å sende inn soknad", brukerId)
